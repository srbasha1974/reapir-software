 ...
 * Needs the records from `06-setup.ts <lang>` (out/06-setup.<lang>.json), or for a re-take of 06 alone
 * `ONLY=06 npx tsx 06-setup.ts <lang>` then `ONLY=06 npx tsx 06-parts.ts <lang>`.
 *
...
/** ONLY=06 reads the records of a `ONLY=06 npx tsx 06-setup.ts <lang>` run (a re-take of 06 alone). */
const setupFile = join(OUT, `06-setup.${lang}${process.env.ONLY === '06' ? '.06' : ''}.json`)
const setup = JSON.parse(readFileSync(setupFile, 'utf8')) as {
  stamp: string
...
/** A click on something the pointer already rests on: the click mark, without a second glide. */
const tap = async (s: Stage, target: Locator) => {
  await s.page.evaluate(() => (window as any).__stage.click())
  await target.click()
  await s.wait(350)
}
...
if (runs(1)) {
  ...
  await s.card(roleCard(c.engK, c.engH), 1200 * pace)
...
if (runs(2)) {
  ...
  await s.say(c.raised, 2400 * pace)
...
if (runs(3)) {
  ...
  await s.say(c.zoho, 2600 * pace)
  await s.say(c.po, 2200 * pace, 'do')
  await s.unring()
  ...
  await s.say(c.ordered, 2200 * pace)
  await s.unring()
  ...
  await s.wait(400 * pace)
  await s.type(p.locator('#supplierName'), 'Sri Murugan', 30)
  ...
  await s.say(c.partial, 2400 * pace, 'do')
  await s.goto('/service-centre/spares?tab=orders')
  await s.point(row().locator('td').nth(3))
  await s.say(c.partialState, 3000 * pace)
  await s.unring()
  await s.quiet()
...
if (runs(4)) {
  ...
  await s.wait(500)
  await s.point(p.locator('.warn.good').first())
  await s.say(c.adjusted, 2400 * pace, 'do')
  await s.unring()
  await s.quiet()
  await s.card(`<div class="k">${c.remember}</div><ol>${c.rules.map((r) => `<li>${r}</li>`).join('')}</ol>`, 5000 * pace)
