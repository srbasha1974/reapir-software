...
await pointAll(s, [normal, line.locator('.handmark')])
...
// The price field is a grouped text box now: select what is there and type over it.
await s.click(price)
await price.press('ControlOrMeta+a')
await price.press('Backspace')
await price.pressSequentially('8000', { delay: 90 })
await s.wait(400)
